...
def addprice(request):
    return render(request,'addpricelist.html')
def addpl(request):
    if request.method=='POST':
        radio=request.POST.get('radio')
        if radio=='rate':
            ...
            history="Created by" + str(us)
            user=User.objects.get(id=u)
            ad_item=pricelist(name=name,types=type,currency=cur,description=desc,user=user
                                )
                ...
                history="Created by" + str(us)
                user=User.objects.get(id=u)
                ad_item=pricelist(name=name,types=type,currency='none',description=desc,user=user,mark=mark,percentage=perc,roundoff=round
                                )
